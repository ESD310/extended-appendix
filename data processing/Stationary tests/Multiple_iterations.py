...
class Target:
    ...
    def update(self, tof: int, sat: int):
        ...
        dx = self.pos_est[0] - last_est[0]
        dy = self.pos_est[1] - last_est[1]
        dz = self.pos_est[2] - last_est[2]
        self.pos_est[0] = last_est[0] + dx
        self.pos_est[1] = last_est[1] + dy
        self.pos_est[2] = last_est[2] + dz
        return self.pos_est
...
def simulate_from_real_meas(measurements: list[pr.Measurement]):
    simulated_measurements: list[pr.Measurement] = []
    for measurement in measurements:
        tof_data = extract_tof(measurement)
        result = run_sim(tof_data)
        result = result.rename(columns={"x": "X", "y": "Y", "z": "Z"})
        result = result.iloc[10:]
        simulated_measurements.append(pr.Measurement(result, measurement.real_pos))
...
if __name__ == "__main__":
    ...
    simulated = simulate_from_real_meas(measurements)
    with open("simulatedobject.obj", 'wb') as f:
        pickle.dump(simulated, f)
    with open("simulatedobject.obj", 'rb') as f:
        simulated = pickle.load(f)



    pr.plot_hist_all(simulated, xmax=2, binsize=0.002)
    pr.plot_hist_all(measurements, xmax=2, binsize=0.002)
    pr.plot_CI(simulated, pr.satellites, points=True)
    #pr.plot_CI(measurements, pr.satellites)
