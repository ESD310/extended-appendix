...
def plot_single(measurement: Measurement, sats, lines=False, iterations=True, sphere=False):
    fig = plt.figure()
    ...
    if(iterations):
        ...
        subplt.text(measurement.real_pos[0], measurement.real_pos[1], measurement.real_pos[2], "True position", color=mark[0].get_color())
        ...
        mark = subplt.plot(measurement.real_pos[0],measurement.real_pos[1],measurement.real_pos[2] - 0.05,  marker='x')
...
def plot_all(measurements: list[Measurement], sats, lines=False, points=False):
    ...
    for measurement in measurements:
        ...
        subplt.plot(measurement.data["X"], measurement.data["Y"], measurement.data["Z"], marker='.', markersize=2,  alpha=0.6, linewidth=1, color=mark[0].get_color()) #  linestyle="dashed"
        if(lines):
            for i in range(0, len(measurement.data)):
                row = measurement.data.iloc[i]
                sat_ID = int(row["ID"])
...
def plot_hist_all(measurements: list[Measurement], xmax=1, binsize=0.02, cum=True, hist=True):
    fig = plt.figure()
    subplt1 = fig.add_subplot()
    subplt2 = subplt1.twinx()  
    ...
    for measurement in measurements:
        errors2 = measurement.Errordistance2mean()
        bin_edges = np.arange(0, xmax, binsize)
        vals, bins = np.histogram(errors2, bins=bin_edges)
        bins = np.append(bins, [10])
        vals = np.append(vals, [vals[len(vals)-1]])
        cum_vals = np.cumsum(vals)/len(errors2)
        if hist:
            subplt1.stairs(vals, bins, linewidth=1, fill=True, alpha=0.3, color="black")
            spl = splrep(bins[0:-1], vals, s=0.002, per=False)
            x_inter = np.linspace(bins[0], bins[-1], 10000)
            y_inter = splev(x_inter, spl)
            subplt1.plot(x_inter,y_inter, linewidth=2)

        if cum:
            style = 'solid'
            if hist:
                style = 'dashed'
            subplt2.stairs(cum_vals, bins, linestyle=style, linewidth=1)

            
    subplt2.set_ylim(0,1)
    if cum:
        subplt2.set_ylabel('Cummulative probability [Dotted]')
        subplt2.plot([0, xmax], [0.95, 0.95], linestyle="dashed", linewidth=0.3,color="r")
    else:
        subplt2.get_yaxis().set_visible(False)
    subplt1.set_ylim(0, max(vals*1.5))
    subplt1.set_xlabel('Distance to most median[m]')
    subplt1.set_xlim(0, xmax)
    if hist:
...
def plot_CI(measurements: list[Measurement], sats, points=False):
    fig = plt.figure()
    subplt = fig.add_subplot(projection="3d")
    for measurement in measurements:
        ...
        if points:
            subplt.scatter(measurement.data["X"],measurement.data["Y"],measurement.data["Z"], color=meanpoint[0].get_color())
...
if __name__ == "__main__":
    ...
    #plot_single(measurents_fixed_z[3], satellites, lines=False, sphere=False)
    #plot_errors_hist(measurents_fixed_z[3])
    plot_hist_all(measurents_fixed_z, xmax=0.1, binsize=0.002, hist=True, cum=False)
    ...
    for measurement in measurents_fixed_z:
        ...
        print("{}: \t SD: {:.3f}, offset: {:.5f}".format(i, measurement.std()*2, error))
        toterror += error
        i += 1
    print(toterror/i)
