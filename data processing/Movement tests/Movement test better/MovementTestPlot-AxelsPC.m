...
data20 = readtable('C:\Users\olsso\OneDrive - Aalborg Universitet\General - ESD310\P3 Projekt\Tests\Movement test better\Test1_speed20.log', ...
...
subplot(2,3,1);
hold on;
plot3(x20_filtered, y20_filtered, z20_filtered, 'bo', 'MarkerFaceColor', 'b'); % Filtrerede punkter
plot3(x20_new, y20_new, z20_new, 'ro', 'MarkerFaceColor', 'r'); % Nye punkter
for i = 1:length(x20_filtered)
    plot3([x20_filtered(i), x20_new(i)], [y20_filtered(i), y20_new(i)], [z20_filtered(i), z20_new(i)], 'k-');
end
title('Speed 20');

subplot(2,3,2);
hold on;
plot3(x40_filtered, y40_filtered, z40_filtered, 'bo', 'MarkerFaceColor', 'b'); % Filtrerede punkter
plot3(x40_new, y40_new, z40_new, 'ro', 'MarkerFaceColor', 'r'); % Nye punkter
for i = 1:length(x40_filtered)
    plot3([x40_filtered(i), x40_new(i)], [y40_filtered(i), y40_new(i)], [z40_filtered(i), z40_new(i)], 'k-');
end
title('Speed 40');

subplot(2,3,3);
hold on;
plot3(x60_filtered, y60_filtered, z60_filtered, 'bo', 'MarkerFaceColor', 'b'); % Filtrerede punkter
plot3(x60_new, y60_new, z60_new, 'ro', 'MarkerFaceColor', 'r'); % Nye punkter
for i = 1:length(x60_filtered)
    plot3([x60_filtered(i), x60_new(i)], [y60_filtered(i), y60_new(i)], [z60_filtered(i), z60_new(i)], 'k-');
end
title('Speed 60');

subplot(2,3,4);
hold on;
plot3(x80_filtered, y80_filtered, z80_filtered, 'bo', 'MarkerFaceColor', 'b'); % Filtrerede punkter
plot3(x80_new, y80_new, z80_new, 'ro', 'MarkerFaceColor', 'r'); % Nye punkter
for i = 1:length(x80_filtered)
    plot3([x80_filtered(i), x80_new(i)], [y80_filtered(i), y80_new(i)], [z80_filtered(i), z80_new(i)], 'k-');
end
title('Speed 80');

subplot(2,3,5);
hold on;
plot3(x100_filtered, y100_filtered, z100_filtered, 'bo', 'MarkerFaceColor', 'b'); % Filtrerede punkter
plot3(x100_new, y100_new, z100_new, 'ro', 'MarkerFaceColor', 'r'); % Nye punkter
for i = 1:length(x100_filtered)
    plot3([x100_filtered(i), x100_new(i)], [y100_filtered(i), y100_new(i)], [z100_filtered(i), z100_new(i)], 'k-');
end
title('Speed 100');

% Juster visningen af plottet
hold off;
